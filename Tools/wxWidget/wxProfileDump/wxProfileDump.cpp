...
void MyFrame::OnAbout(wxCommandEvent& WXUNUSED(event) )
{
  (void)wxMessageBox(_T("wxProfileDump\n")
    _T("Copyright (C) 2005-2006\n"),
    _T("About wxProfileDump"));
}

void MyFrame::OnOpenProfile(wxCommandEvent& WXUNUSED(event) )
{
  ...
  if (dialog.ShowModal()!=wxID_OK)
    ...
    wxString profilePath = dialog.GetPath();
  wxString profileTitle = dialog.GetFilename();
